...
@section('content')
    <div class="container">
        ...
        <div class="flex w-full justify-end items-center my-2">
            <a href="{{ route('sales.create') }}" class="bg-blue-500 flex space-x-3 justify-center items-center p-1">
                    ...
                    <tr class="text-center">
                        ...
                        <td class="p-4 align-middle">
                            <div class="flex justify-center items-center">
                                <img src="{{ Storage::url($man->photo) }}" alt="{{ $man->name }}"
                                    class="h-[40px] w-[30px] bg-gray-400 object-cover object-center">
                            </div>
                        </td>

                        <!-- Tipe Mobil -->
                        <td class="ml-2 align-middle text-left">{{ $man->name }}</td>

                        <!-- Nomor Mesin -->
                        <td class="ml-2 align-middle text-left">{{ $man->phone_number }}</td>

                        <!-- Ikon untuk edit dan hapus -->
                        <td class="px-5 align-middle">
                            <div class="flex space-x-5 justify-center items-center">
                                <!-- Link Hapus -->
                                <form action="{{ route('sales.destroy', ['sale' => $man->id]) }}" method="POST"
                                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus sales ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit">
                                ...
                                <!-- Link Edit -->
                                <a href="{{ route('sales.edit', ['sale' => $man->id]) }}">
