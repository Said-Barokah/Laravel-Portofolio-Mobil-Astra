...
@section('content')
    <div class="container">
        ...
        <div class="flex w-full justify-end items-center my-2">
            <a href="{{ route('vehicleColor.create') }}" class="bg-blue-500 flex space-x-3 justify-center items-center p-1">
        ...
        <table class="table-auto w-full">
            ...
            <tbody class="bg-blue-50">
                @foreach ($vehicleColors as $index => $vehicleColor)
                    <tr class="text-center">
                        ...
                        <td class="p-4 align-middle">
                            <div class="flex justify-center items-center">
                                <img src="{{ Storage::url($vehicleColor->photo) }}" alt="{{ $vehicleColor->vehicle->name }}"
                                    class="h-[40px] w-[30px] bg-gray-400 object-cover object-center">
                            </div>
                        </td>

                        <!-- Tipe Mobil -->
                        <td class="ml-2 align-middle text-left">{{ $vehicleColor->vehicle->name }}</td>

                        <!-- Nomor Mesin -->
                        <td class="ml-2 align-middle text-left">{{ $vehicleColor->color->name }}</td>

                        <!-- Ikon untuk edit dan hapus -->
                        <td class="px-5 align-middle">
                            <div class="flex space-x-5 justify-center items-center">
                                <!-- Link Hapus -->
                                <form action="{{ route('vehicleColor.destroy', ['vehicle_color' => $vehicleColor->id]) }}" method="POST"
                                    onsubmit="return confirm('Apakah Anda yakin ingin menghapus warna kendaraan ini?')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit">
                                ...
                                <!-- Link Edit -->
                                <a href="{{ route('vehicleColor.edit', ['vehicle_color' => $vehicleColor->id]) }}">
        ...
        <section class="flex justify-center items-center space-x-4 mt-6">
            <!-- Pagination links -->
            {{ $vehicleColors->links('vendor.pagination.tailwind') }}
        </section>
    </div>
@endsection
