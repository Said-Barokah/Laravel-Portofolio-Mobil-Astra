...
@section('content')
    <div class="container">
        ...
        <div class="flex w-full justify-end items-center my-2">
            <a href="{{ route('discount.create') }}" class="bg-blue-500 flex space-x-3 justify-center items-center p-1">
                <p class="text-white font-semibold text-lg rounded-md p-2">+ Tambah Diskon</p>
            </a>
        </div>
        <table class="table-auto w-full">
            <thead>
                <tr>
                    ...
                    <th class="text-left">Tipe Diskon</th>
            ...
            <tbody class="bg-blue-50">
                @foreach ($discounts as $index => $discount)
                    <tr class="text-center">
                        ...
                        <!-- Tipe Mobil -->
                        <td class="ml-2 align-middle text-left">{{ $discount->name }}</td>

                        <!-- Nomor Mesin -->
                        <td class="ml-2 align-middle text-left">{{ $discount->type }}</td>
                        <td class="ml-2 align-middle text-left">{{ $discount->amount }}</td>
                        ...
                        <td class="px-5 align-middle">
                            <div class="flex space-x-5 justify-center items-center">
                                <!-- Link Hapus -->
                                <form action="{{ route('discount.destroy', ['discount' => $discount->id]) }}" method="POST"
