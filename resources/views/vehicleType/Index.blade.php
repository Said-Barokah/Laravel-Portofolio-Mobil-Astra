...
@section('content')
    <div class="container">
        ...
        <div class="flex w-full justify-end items-center my-2">
            <a href="{{ route('vehicleType.create') }}" class="bg-blue-500 flex space-x-3 justify-center items-center p-1">
            ...
            <tbody class="bg-blue-50">
                @foreach ($vehicleTypes as $index => $vehicleType)
                    <tr class="text-center">
                        ...
                        <td class="ml-2 align-middle text-left">{{ $vehicleType->type }}</td>

                        <td class="ml-2 align-middle text-left">{{ \Illuminate\Support\Str::limit($vehicleType->description, 50, '...') }}</td>

                        <!-- Ikon untuk edit dan hapus -->
                        <td class="px-5 align-middle">
                            <div class="flex space-x-5 justify-center items-center">
                                <!-- Link Hapus -->
                                <form action="{{ route('vehicleType.destroy', ['vehicle_type' => $vehicleType->id]) }}" method="POST"
                                ...
                                <!-- Link Edit -->
                                <a href="{{ route('vehicleType.edit', ['vehicle_type' => $vehicleType->id]) }}">
        ...
        <section class="flex justify-center items-center space-x-4 mt-6">
            <!-- Pagination links -->
            {{ $vehicleTypes->links('vendor.pagination.tailwind') }}
        </section>
    </div>
@endsection
