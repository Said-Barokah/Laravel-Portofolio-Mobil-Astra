        ...
        <form action="{{ route('promoCode.store') }}" method="POST" enctype="multipart/form-data"
            ...
            <div class="flex flex-col space-y-3">
                ...
                <select name="discount_id" id="discount_id" class="w-full border-2 border-gray-200 rounded-lg p-3 text-gray-700">
                    ...
                    <option value="{{$discount->id}}">{{ $discount->name}} - Diskon sebesar {{ $discount->amount  }}%</option>
                    @elseif ($discount->type == 'Nominal')
                    <option value="{{$discount->id}}">{{ $discount->name}} - Diskon sebesar Rp{{ number_format($discount->amount, 0, ',', '.')  }}</option>
                    @endif
                    @endforeach
                </select>
            </div>
