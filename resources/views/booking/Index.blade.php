...
@section('content')
    <div class="container">
        ...
        <div class="flex w-full justify-end items-center my-2">
            <a href="{{ route('booking.create') }}" class="bg-blue-500 flex space-x-3 justify-center items-center p-1">
                <p class="text-white font-semibold text-lg rounded-md p-2">+ Tambah Booking</p>
            </a>
        </div>
            ...
            <tbody class="bg-blue-50">
                @foreach ($bookings as $index => $booking)
                    <tr class="text-center">
                        ...
                        <!-- Nama Customer -->
                        <td class="ml-2 align-middle">{{ $booking->customer->name }}</td>

                        <!-- Tipe Mobil -->
                        <td class="ml-2 align-middle">{{ $booking->vehicleDetail->vehicleColor->vehicle->name }}</td>

                        <!-- Nomor Mesin -->
                        <td class="ml-2 align-middle">{{ $booking->vehicleDetail->chassis_number }}</td>

                        <!-- Warna Mobil -->
                        <td class="ml-2 align-middle">{{ $booking->vehicleDetail->vehicleColor->vehicle->color }}</td>

                        <!-- Tanggal Pemesanan -->
                        <td class="pr-5 align-middle">{{ $booking->created_at->format('Y-m-d') }}</td>

                        <!-- Status Booking -->
                        <td class="pr-5 align-middle">{{ $booking->status }}</td>
